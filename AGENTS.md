...
state/               runtime records and signals; gitignored
  ...
  .<id>.open-decisions-presented  per-task receipt of the keyed decisions actually shown by the bounded OPEN DECISIONS presentation; written by the locked wake drain, consumed by self-announced answer closes only when its prefix and file identity match, and removed by teardown
